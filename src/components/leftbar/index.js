...
function LeftBar({ info, setInfo }) {
  const playlistLink =
    "https://open.spotify.com/playlist/0rXXhdpACJCsvCpCSa513D?si=89e4bb5f0be64c85";
  return (
    <div className="left">
      <div
        onClick={() => setInfo(false)}
        className={info === true ? "option-left" : "option-left select"}
      >
        <img
          src={info === true ? homeIconWhite : homeIconBlack}
          alt="home icon"
        />
        <h1>Home</h1>
      </div>
      <div
        onClick={info === true ? () => setInfo(false) : () => setInfo(true)}
        className={info ? "option-left select" : "option-left "}
      >
